...
- `BankService.payout_summary`: per-kid row now embeds the full `outstanding_weeks` list (via `list_outstanding_weeks`) instead of a single current-week snapshot. `paycheck_total_cents` sums every outstanding week's amount across every kid — the "Total owed" figure on `/parent/payouts` becomes honest about backlog, not just this week.
