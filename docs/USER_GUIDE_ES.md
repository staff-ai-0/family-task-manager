...
```mermaid
graph TD
    A[Family Task Manager] --> B[Tareas]
    A --> C[Premios]
    A --> D[Consecuencias]
    A --> E[Presupuesto]
    A --> F[Configuracion]
...
```mermaid
graph TD
    A[Padre se Registra] --> B[Crea la Familia]
    B --> C{Invitar Miembros}
    C -->|Por correo| D[Registrar Nuevo Miembro]
    C -->|Por codigo| E[Generar Codigo de Invitacion]
    D --> F[Miembro Activo]
    E --> G[Miembro Ingresa Codigo]
    G --> F
    F --> H[Asignar Rol]
    H --> I[Hijo/a]
    H --> J[Adolescente]
...
```mermaid
graph LR
    A[Crear Plantilla] --> B[Asignar Semanal]
    B --> C[Tarea Pendiente]
    C --> D[Hijo Completa]
    D --> E[Puntos Otorgados]
    E --> F[Canjear Premios]
```
...
```mermaid
graph TD
    A[Gestionar Tareas] --> B[Ver Todas las Plantillas]
    B --> C{Accion}
    C -->|Crear| D[Llenar Formulario]
    C -->|Editar| E[Modificar Campos]
    C -->|Eliminar| F[Confirmar Eliminacion]
    C -->|Toggle| G[Activar o Desactivar]
    D --> H[Crear Plantilla]
    E --> I[Guardar Cambios]
    H --> J[Plantilla Lista para Asignar]
    I --> J
```
...
```mermaid
graph LR
    A[Ganar Puntos con Tareas] --> B[Abrir Tienda de Premios]
    B --> C[Seleccionar Premio]
    C --> D{Puntos Suficientes?}
    D -->|Si| E[Canjear Premio]
    D -->|No| F[Seguir Completando Tareas]
    E --> G[Puntos Descontados]
    G --> H[Premio Obtenido]
```
...
```mermaid
graph TD
    A[Miembro Incumple] --> B[Padre Crea Consecuencia]
    B --> C[Consecuencia Activa]
    C --> D[Miembro la Ve en Perfil]
    D --> E[Miembro Cumple la Sancion]
    E --> F[Padre Resuelve]
    F --> G[Consecuencia Resuelta]
```
...
```mermaid
graph TD
    A[Ingreso Mensual] --> B[Listo para Asignar]
    B --> C[Sobre: Renta]
    B --> D[Sobre: Comida]
    B --> E[Sobre: Transporte]
    B --> F[Sobre: Ahorro]
    C --> G[Gastar de Renta]
    D --> H[Gastar de Comida]
    E --> I[Gastar de Transporte]
    G --> J[Revisar al Final del Mes]
    H --> J
    I --> J
    F --> J
```
...
```mermaid
graph LR
    A[Inicio de Presupuesto] --> B[Presupuesto Mensual]
    A --> C[Cuentas]
    A --> D[Movimientos]
    A --> E[Categorias]
    A --> F[Reportes]
...
```mermaid
graph TD
    A[Obtener Estado de Cuenta] --> B[Abrir Conciliacion]
    B --> C[Marcar Transacciones Aclaradas]
    C --> D{Saldo Coincide?}
    D -->|Si| E[Confirmar Conciliacion]
    D -->|No| F[Buscar Discrepancias]
    F --> C
    E --> G[Transacciones Bloqueadas]
```
...
```mermaid
graph TD
    P["Transaccion Padre: Soriana $1,500"] --> C1["Hijo 1: Comida $1,200"]
    P --> C2["Hijo 2: Limpieza $300"]
```
...
```mermaid
graph TD
    A[Descargar CSV del Banco] --> B[Ir a Importar]
    B --> C[Seleccionar Archivo CSV]
    C --> D[Configurar Delimitador]
    D --> E[Mapear Columnas]
    E --> F[Vista Previa de Datos]
    F --> G[Seleccionar Cuenta Destino]
    G --> H[Importar]
    H --> I[Revision de Resultados]
    I --> J[Transacciones Disponibles]
```
...
```mermaid
graph TD
    A[Categorias] --> B[Grupo: Gastos Fijos]
    A --> C[Grupo: Alimentacion]
    A --> D[Grupo: Transporte]
    A --> E[Grupo: Ingresos]
    B --> B1[Renta]
    B --> B2[Servicios]
    B --> B3[Internet]
...
```mermaid
graph TD
    A[Inicio de Mes] --> B[Recibir Ingresos]
    B --> C[Ver Listo para Asignar]
    C --> D[Asignar a Categorias]
    D --> E[Gastar Durante el Mes]
    E --> F{Categoria Sobregastada?}
    F -->|Si| G[Transferir de Otra Categoria]
    F -->|No| H[Continuar]
    G --> H
    H --> I[Fin de Mes: Revisar]
    I --> J[Cerrar Mes]
    J --> K[Siguiente Mes: Rollover]
```
...
```mermaid
graph TD
    A[Nueva Transaccion] --> B[Buscar Reglas por Prioridad]
    B --> C{Regla 1: Coincide?}
    C -->|Si| D[Asignar Categoria]
    C -->|No| E{Regla 2: Coincide?}
    E -->|Si| D
    E -->|No| F{Regla N: Coincide?}
    F -->|Si| D
    F -->|No| G[Sin Categoria]
```
...
```mermaid
graph TD
    A[Crear Plantilla Recurrente] --> B[Configurar Frecuencia]
    B --> C[Sistema Registra Proxima Fecha]
    C --> D{Llego la Fecha?}
    D -->|Si| E[Generar Transaccion Automatica]
    D -->|No| F[Esperar]
    E --> G[Transaccion en la Cuenta]
    G --> H[Afecta Presupuesto]
    H --> I[Calcular Siguiente Fecha]
    I --> D
```
...
```mermaid
graph TD
    A[Abrir Reporte de Gastos] --> B[Seleccionar Periodo]
    B --> C[Filtrar por Categoria]
    C --> D[Ver Grafico y Tabla]
    D --> E[Analizar Patrones]
    E --> F[Ajustar Presupuesto]
```
