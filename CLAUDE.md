...
- `/api/task-templates/` + `/api/task-assignments/` — the task system (the pre-2026 legacy `/api/tasks` system was fully removed 2026-07-16: code, model, and — via the `drop_legacy_tasks` migration — its table). Parent review is **graded**: `ApprovalDecision.grade` = `full` / `partial` (1–99%, default 50) / `missed`; point awards scale by the grade (integer half-up) and `approval_notes` surfaces to the kid. Partial is rejected on collaboration gigs (pot conservation). **Wording rule**: this review queue is chores + bonus tasks (points) — user-facing copy says "task/tarea"; the word "gig" is reserved for the cash gig board (`gig_claim_*` keys).
  ...
  **The weekly shuffle balancer is multi-objective** (PR #224, after three point-only "fixes" each looked correct while the board still read unbalanced): POINTS are primary (spread ≤ one biggest template), chore COUNT is secondary (point-neutral bundle swaps: two small chores ↔ one big; bounded single-chore fallback), pinned rotations carry a turn cap `ceil(occurrences/list_size)` on every balancing move (AUTO/listless deliberately uncapped or parity deadlocks), and a day pass spreads each member's chores via equal-point cross-template swaps. Triaging "shuffle is unbalanced": compare BOTH metrics against the DB first — the board shows counts, parity is points — and check the family's template config: if a member's receivable chores are all one denomination (prod 2026-08-10: both 20-pt rotations excluded one parent), point parity mathematically forces their count up and no balancer change can fix it. Invariant tests: `test_shuffle_count_balance.py` (seed-parametrized, includes an exact prod-family mirror).
...
- **Frontend is a sheet, not a page**: `frontend/src/components/budget/ReceiptScanSheet.astro`, mounted once by `BudgetShell`, so any budget page can scan without navigating. Open it with `[data-scan-receipt-open]` or a `ftm:scan-receipt` window event; `input.click()` must stay synchronous with the originating gesture (iOS drops a picker opened after an `await`). ONE file input, deliberately **without `capture`** — that attribute pins iOS to the camera, which is why the old page needed separate camera/upload/bulk buttons; `multiple` on the same input drives the bulk path. `/budget/scan-receipt` is a redirect to `/budget/transactions?scan=1` (link-compat for email links + no-JS fallbacks, no UI of its own). **Visible scan triggers are exactly two: the FAB's "Escanear" mode and the drawer's "Escanear ticket"** — settled 2026-08-11 after two user complaints in one day: first "duplicated scan receipt" (standalone page + header camera + FAB), then "can't take a pic" when the FAB button was the one removed. The FAB is where users look for it; don't re-add a header camera icon or a standalone scan page.
