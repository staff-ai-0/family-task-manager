...
class Task(Base):
    ...
    )  # Required task vs optional
    frequency = Column(
        SQLEnum(TaskFrequency), default=TaskFrequency.DAILY, nullable=False
    )
    status = Column(
    ...
    consequence = relationship(
        "Consequence", back_populates="triggered_task", uselist=False
    )
    point_transactions = relationship(
        "PointTransaction", back_populates="task", cascade="all, delete-orphan"
    )
