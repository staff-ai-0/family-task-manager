...
/**
 * Wildcard proxy for all /api/family-cup/* requests.
 *
 * Browser-side JS holds only the httpOnly access_token cookie and cannot reach
 * the backend directly, so this route forwards every method transparently and
 * lifts the cookie into an Authorization header. Mirrors api/gigs/[...path].ts,
 * and additionally forwards the Cloudflare client IP so the backend's per-user
 * rate-limit buckets stay intact (see lib/client-ip.ts).
 *
 * Route: /api/family-cup/[...path]  →  <BACKEND>/api/family-cup/<path>
 */
async function proxy({ request }: { request: Request; params: Record<string, string | undefined> }): Promise<Response> {
    ...
    try {
        ...
        console.error(`[api/family-cup proxy] Error forwarding to ${backendUrl}:`, e?.message ?? e);
        return new Response(
