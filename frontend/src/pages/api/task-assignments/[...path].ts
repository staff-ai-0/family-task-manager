...
/**
 * Wildcard proxy for all /api/task-assignments/* requests.
 *
 * Quest Mode (/pet/quests) completes assignments over AJAX so it can show an
 * instant pet reaction instead of a full-page redirect. Browser JS holds only
 * the httpOnly access_token cookie and cannot reach the backend directly, so
 * this route forwards every method transparently, lifts the cookie into an
 * Authorization header, and forwards the Cloudflare client IP for the backend
 * rate limiter. Unlike api/assignments/complete.ts (which 302-redirects to the
 * dashboard), this proxy streams the backend's JSON straight back to the
 * caller. Mirrors api/family-cup/[...path].ts.
 *
 * Route: /api/task-assignments/[...path]  →  <BACKEND>/api/task-assignments/<path>
 */
async function proxy({ request }: { request: Request; params: Record<string, string | undefined> }): Promise<Response> {
    ...
    try {
        ...
        console.error(`[api/task-assignments proxy] Error forwarding to ${backendUrl}:`, e?.message ?? e);
        return new Response(
