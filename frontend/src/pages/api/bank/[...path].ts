...
/**
 * Wildcard proxy for all /api/bank/* requests (Family Bank — jars, transfers,
 * settings, kid requests).
 *
 * Browser JS only holds the httpOnly access_token cookie; the backend needs a
 * Bearer header. This forwards every method and injects the cookie as
 * Authorization. Mirrors api/cash/[...path].ts.
 *
 * Route: /api/bank/[...path]  →  <BACKEND>/api/bank/<path>
 */
async function proxy({ request, params }: { request: Request; params: Record<string, string | undefined> }): Promise<Response> {
    ...
    try {
        ...
        console.error(`[api/bank proxy] Error forwarding to ${backendUrl}:`, e?.message ?? e);
        return new Response(
