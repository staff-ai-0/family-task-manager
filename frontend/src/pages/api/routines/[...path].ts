...
/**
 * Wildcard proxy for all /api/routines/* requests.
 *
 * The routines runner (kid taps steps, parent authors routines) is driven
 * client-side, but browser JS holds only the httpOnly access_token cookie and
 * cannot reach the backend directly. This route forwards every method
 * transparently, lifts the cookie into an Authorization header, and forwards
 * the Cloudflare client IP so the backend's per-user rate-limit buckets stay
 * intact. Mirrors api/pet/[...path].ts.
 *
 * Route: /api/routines/[...path]  →  <BACKEND>/api/routines/<path>
 */
async function proxy({ request }: { request: Request; params: Record<string, string | undefined> }): Promise<Response> {
    ...
    try {
        ...
        console.error(`[api/routines proxy] Error forwarding to ${backendUrl}:`, e?.message ?? e);
        return new Response(
